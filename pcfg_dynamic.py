...
def word_categories(s: list, t_rules: dict):
    ...
    :param s: sentence (ordered list of words)
    :param t_rules: terminal rules (dictionary of "CAT" [(word, probability), (word2, probability2),...])
    :return: sentence transformed into list of tuples (word, category).
    """
    cat = {}
...
def cky_algorithm(sentence: list, t_rules: dict, nt_rules: dict):
    ...
    Table of probabilities and subtrees grammatical categories are displayed after execution.

    :param sentence: list of words
    :param t_rules: terminal rules (dictionary of "CAT": [(word, probability), (word2, probability2),...])
    :param nt_rules: non-terminal rules (dictionary of "YZ": (X, probability) for rules X -> YZ)
    :return: nothing, information generated by the algorithm is printed at the end.
    """
...
def inside_algorithm(sentence: list, t_rules: dict, nt_rules: dict):
    ...
    Table of probabilities is displayed after execution.
...
def init_table(s: list, t_rules: dict):
    ...
    :param t_rules: terminal rules (dictionary of "CAT": [(word, probability), (word2, probability2),...])
    :return: tuple of (table of probabilities, table of terminal nodes' categories)
    """
    s_length = len(s)
...
def print_table(s: list, table: numpy.array):
    ...
    :param s: list of words
    :param table: table of floats to be printed.
    :return: nothing
    """
    length = len(s)
    if length != len(table):
...
def print_str_table(s: list, table: np.array):
    ...
    :param s: list of words
    :param table: table of strings to be printed.
    :return: nothing
    """
    length = len(s)
    if length != len(table):
...
def reconstruct_tree(t_dict: dict, cat_table: np.array, node: tuple, s: list):
    """
    Recursive algorithm to reconstruct the tree from the pointers saved during parsing,
    and assign words from sentence to terminal nodes.

    :param t_dict: dictionary of nodes "X": (X, Y, Z) for rules X -> Y Z
    :param cat_table: category table for the sentence
    :param node: current node (starts with S)
    :param s: sentence used for parsing
    :return:
    """
    ...
    # Terminal node on left side : print word from sentence.
    elif right_node is not None:
        print(s[node[1][0]])
    if right_node is not None:
        reconstruct_tree(t_dict, cat_table, right_node, s)
    # Terminal node on right side : print word from sentence.
    elif left_node is not None:
        print(s[node[2][0]])
    # Both sides are terminal : print words from sentence.
